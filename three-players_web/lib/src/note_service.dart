import 'dart:async';
import 'dart:convert';

import 'package:http/http.dart';

import 'note.dart';
import 'mock_notes.dart';
   
class NoteService {
  static final _headers = {'Content-Type': 'application/json'};
  static const _notesUrl = 'api/notes'; // URL to web API

final Client _http;

NoteService(this._http);

Future<List<Note>> getAll() async {
  try {
    final response = await _http.get(_notesUrl);
    final notes = (_extractData(response) as List)
        .map((json) => Note.fromJson(json))
        .toList();
    return notes;
  } catch (e) {
    throw _handleError(e);
  }
}

dynamic _extractData(Response resp) => json.decode(resp.body)['data'];

Exception _handleError(dynamic e) {
  print(e); // for demo purposes only
  return Exception('Server error; cause: $e');
}

  Future<Note> get(int id) async {
    try {
      final response = await _http.get('$_notesUrl/$id');
      return Note.fromJson(_extractData(response));
    } catch (e) {
      throw _handleError(e);
    }
  }

  Future<Note> create(String content) async {
    try {
      final response = await _http.post(_notesUrl,
          headers: _headers, body: json.encode({'content': content}));
      return Note.fromJson(_extractData(response));
    } catch (e) {
      throw _handleError(e);
    }
  }

  Future<Note> update(Note note) async {
    try {
      final url = '$_notesUrl/${note.id}';
      final response =
          await _http.put(url, headers: _headers, body: json.encode(note));
      return Note.fromJson(_extractData(response));
    } catch (e) {
      throw _handleError(e);
    }
  }

  Future<void> delete(int id) async {
    try {
      final url = '$_notesUrl/$id';
      await _http.delete(url, headers: _headers);
    } catch (e) {
      throw _handleError(e);
    }
  }
}



